...
local BN,parent = torch.class('nn.initSBatchNormalization', 'nn.Module')
...
function BN:__init(nOutput, eps, momentum, affine)
   ...
   assert(nOutput ~= 0, 'To set affine=false call initSBatchNormalization'
...
function BN:updateOutput(input)
   ...
   self.save_mean = self.running_mean:clone()
   self.save_std = self.running_var:clone()
