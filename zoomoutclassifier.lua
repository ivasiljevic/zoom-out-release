...
function zoomoutclassifier(origstride,nlabels,nhiddenunits,inputsize)
    ...
    mod:add(nn.ReLU(true))
    mod:add(nn.Dropout(0.5))
    mod:add(nn.SpatialConvolutionMM(nhiddenunits,23, 1, 1))
    ...
    mod:add(nn.SpatialFullConvolution(23,nlabels,origstride*2,origstride*2,origstride,origstride,origstride/2,origstride/2))
    local W = Bilinearkernel(origstride*2,nlabels,23)  -- initailization to bilinear upsampling
